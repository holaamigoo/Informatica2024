...
tuple<vector<Coord>, double, vector<string>> dijkstra(Graph& graph, Coord& start, Coord& end) {
...
tuple<map<string, Coord>, vector<EdgeItem>> read_graphml(const string& file_path) {
    ...
    for (tinyxml2::XMLElement* edge = graph->FirstChildElement("edge"); edge; edge = edge->NextSiblingElement("edge")) {
        ...
        for (tinyxml2::XMLElement* data = edge->FirstChildElement("data"); data; data = data->NextSiblingElement("data")) {
            ...
            if (strcmp(key, "d13") == 0 && data->GetText()) { // street name
...
int main() {
    ...
    map<string, Coord> nodes;
    vector<EdgeItem> edges;
    std::tie(nodes, edges) = read_graphml("oslo_road_network.graphml");
    //print_edges(edges);
    // 2. Задаём названия улиц для начала и конца маршрута
    Coord start, end;
    string name;
    std::tie(start, end, name) = edges[edges.size() - 1];
    string start_street_query = "Kjelsaslokka";      // Название улицы для старта (пример)
    string end_street_query = "Frognerseterveien";      // Название улицы для финиша (пример)
    ...
    if (start_index == -1 || end_index == -1) {
        ...
        auto [result, execution_time] = measure_execution_time(dijkstra, graph, start_node, end_node);
